...
;
; This file contains all necessary functions for different type of searches
;
;
; To test this code, please use: ehsan20.test.search.v03.lisp
;
...
; nodepth limit, if set to nil ... Otherwise the depth limit would be applied for the search
(setf *depth-limit* nil)

; The default function names for a best search algorithm,
; If a new function is needed for a certain application, the name of the function
; would be pass to its corresponding variable
...
; it returns the priority of the give node
...
; closed: a given list of nodes that contains all the discovered nodes
; that are not the solution of the problem
; state: a new state
; If a state has be already discovered, i.e. available in the "closed" list" --> returns True
...
;
; input: goal-function
; The name of the function that determines we've achieved our goal
...
(defun depth-first-recur (goal-function)
   ...
   (if (equal current-node nil)
	...
	; testing if the goal is achieved based on the "goal-function"
...
; h-function, g-function, f-function are the default evaluation functions,
; This way we provide more flexibilities for users to define their own functions
; 

; by default, the heuristic function is set to zero
...
; the default g-function, that returns the level of a node
...
; it pops a member from *open* list that has the highest priority, i.e. lowest f-value
; The *open* list would be modified and the popped element would be returned
;
; Dependency: pop-open-priority-recur
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; it checks if a given "node' can be inserted between a head list of nodes and a tail list of nodes
; The idea is to recursively break the *open* list into head and tail to find the 
; best place to insert a new node based on its priority
; This way *open* list would be always sorted ---> this is used for a best-search algorithm
...
; Given a "list-node", i.e. a list of new nodes, this function insert all of them
; into *open* list based on their priorities
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defun best-first-recur (goal-function)
   ...
   (if (equal current-node nil)
	...
	(if (funcall goal-function (stateofnode current-node) *goal*)
	    ...
	    ; else - 
	    ; 1) We are done with the current node - add it to the *closed" list
	    ; 2) Add the children of the current node the *open* list, while keep it sorted
	    ; 3) recursively continue the process
	    (progn
...
; these two functions are exactly the same as "depth-first" and "depth-first-recur"
...
; returns a list of nodes which are children of a given "state"
; 
; Function Dependencies:
;   1) *moves* is the functions that defines the plausible children of the 
;   given state based on the rules of a game
;   2) children-recur  --> for recursion over all possible *moves*
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; It recursively checks all the possible moves to
; create the children list of a given state
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
